...
export const TokenCounter: Plugin = async () => {
  ...
  if (htmlPath) {
    ...
    console.error("[Token Counter] Dashboard HTML not found at", join(__dirname, "web", "index.html"));
  }
  ...
  if (welcomePending) {
    ...
    console.error("");
    console.error("================================================");
    console.error("  Token Counter v0.2.0 installed!");
  ...
  return {
    config: async (cfg: any) => {
      ...
      cfg.command["tokens-export"] = { description: "Export token usage as CSV", template: "/tokens export 7d" };
    },
    ...
    tool: {
      tokens: tool({
        description: "Get token usage and cost statistics. Call when user asks about token usage, spending, or AI costs.",
        args: {
